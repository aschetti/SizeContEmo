---
title: "<center> <h1>***SIZECONTEMO_EEG***</h1> </center>"
...
grand.avg.localizer.pointsummary$timepoint <- as.numeric(levels(grand.avg.localizer.pointsummary$timepoint))[grand.avg.localizer.pointsummary$timepoint] # convert time points to number
...
# rename levels
levels(grand.avg.comps$condition) <- c("localizer","negative","negative minus neutral","negative large bright","negative large dark","negative small bright","negative small dark","neutral","neutral large bright","neutral large dark","neutral small bright","neutral small dark")
# main effect of size
grand.avg.comps$size <- revalue(factor(grand.avg.comps$condition),c("localizer"=NA,
                                                              "negative"=NA,
...
grand.avg.comps$cont <- revalue(factor(grand.avg.comps$condition),c("localizer"=NA,
                                                              "negative"=NA,
                                                              "negative minus neutral"=NA,
...
grand.avg.comps$emo <- revalue(factor(grand.avg.comps$condition),c("localizer"=NA,
                                                             "negative"=NA,
                                                             "negative minus neutral"=NA,
...
P1.grand.avg.comps.pointsummary$timepoint <- as.numeric(levels(P1.grand.avg.comps.pointsummary$timepoint))[P1.grand.avg.comps.pointsummary$timepoint] # convert time points to number
...
ggplot(filter(P1.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  theme_pander(base_size=20,pc="white",lp=c(.15,.8)) # custom theme
```
...
N1.grand.avg.comps.pointsummary <- grand.avg.comps %>% # data frame
  ...
  summarySEwithin(data=.,measurevar="amplitude",withinvars=c("condition","timepoint"),idvar="participant") # summary
N1.grand.avg.comps.pointsummary$timepoint <- as.numeric(levels(N1.grand.avg.comps.pointsummary$timepoint))[N1.grand.avg.comps.pointsummary$timepoint] # convert time points to number
...
EPN.grand.avg.comps.pointsummary <- grand.avg.comps %>% # data frame
  ...
  summarySEwithin(data=.,measurevar="amplitude",withinvars=c("condition","timepoint"),idvar="participant") # summary
EPN.grand.avg.comps.pointsummary$timepoint <- as.numeric(levels(EPN.grand.avg.comps.pointsummary$timepoint))[EPN.grand.avg.comps.pointsummary$timepoint] # convert time points to number
...
LPP.grand.avg.comps.pointsummary <- grand.avg.comps %>% # data frame
  ...
  summarySEwithin(data=.,measurevar="amplitude",withinvars=c("condition","timepoint"),idvar="participant") # summary
LPP.grand.avg.comps.pointsummary$timepoint <- as.numeric(levels(LPP.grand.avg.comps.pointsummary$timepoint))[LPP.grand.avg.comps.pointsummary$timepoint] # convert time points to number
...
<center> <h1>*CONFIRMATORY ANALYSIS*</h1> </center>

```{r data}
data.EEG.trial <- read.csv(paste0(expphase,"_trialEEG.csv"),header=TRUE) # load data
data.EEG.trial$condition <- factor(data.EEG.trial$condition) # convert condition numbers as factors
data.EEG.trial$size <- revalue(factor(data.EEG.trial$condition),c("1"="large","2"="small","3"="large","4"="small","5"="large","6"="small","7"="large","8"="small")) # main effect of size
data.EEG.trial$cont <- revalue(factor(data.EEG.trial$condition),c("1"="dark","2"="dark","3"="bright","4"="bright","5"="dark","6"="dark","7"="bright","8"="bright")) # main effect of contrast
data.EEG.trial$emo <- revalue(factor(data.EEG.trial$condition),c("1"="negative","2"="negative","3"="negative","4"="negative","5"="neutral","6"="neutral","7"="neutral","8"="neutral")) # main effect of emotion
data.EEG.trial$condition <- revalue(factor(data.EEG.trial$condition),c("1"="negativeLargeDark","2"="negativeSmallDark","3"="negativeLargeBright","4"="negativeSmallBright","5"="neutralLargeDark","6"="neutralSmallDark","7"="neutralLargeBright","8"="neutralSmallBright")) # recode bin variable
...
EPN.posthocBF.emo <- data.EEG.trial %>% # data frame
  select(participant,emo,) %>% # select only variables of interest
...
<center> <h1>*SUPPLEMENTARY ANALYSIS*</h1> </center>
...
data.P1.peaks <- data.P1.peakamp %>% # data frame
  select(participant=ERPset,component=chlabel,condition=binlabel,condnum=bini) %>% # select and reorder variables
  mutate(condnum=factor(condnum), # convert condition numbers as factors
         size=revalue(condnum,c("1"="large","2"="small","3"="large","4"="small","5"="large","6"="small","7"="large","8"="small")), # main effect of size
         size=relevel(size,ref="large"), # re-reference size to large
         cont=revalue(condnum,c("1"="dark","2"="dark","3"="bright","4"="bright","5"="dark","6"="dark","7"="bright","8"="bright")), # main effect of contrast
         cont=relevel(cont,ref="dark"), # re-reference contrast to dark
         emo=revalue(condnum,c("1"="negative","2"="negative","3"="negative","4"="negative","5"="neutral","6"="neutral","7"="neutral","8"="neutral")), # main effect of emotion
         emo=relevel(emo,ref="neutral"), # re-reference emotion to neutral
         peak.amp=data.P1.peakamp$value, # peak amplitude
         peak.lat=data.P1.peaklat$value) %>% # peak latency
    select(-condnum) # eliminate variable with conditions as numbers

data.N1.peaks <- data.N1.peakamp %>% # data frame
  select(participant=ERPset,component=chlabel,condition=binlabel,condnum=bini) %>% # select and reorder variables
  mutate(condnum=factor(condnum), # convert condition numbers as factors
         size=revalue(condnum,c("1"="large","2"="small","3"="large","4"="small","5"="large","6"="small","7"="large","8"="small")), # main effect of size
         size=relevel(size,ref="large"), # re-reference size to large
         ...
         emo=revalue(condnum,c("1"="negative","2"="negative","3"="negative","4"="negative","5"="neutral","6"="neutral","7"="neutral","8"="neutral")), # main effect of emotion
         emo=relevel(emo,ref="neutral"), # re-reference emotion to neutral
         peak.amp=data.N1.peakamp$value, # peak amplitude
         peak.lat=data.N1.peaklat$value) %>% # peak latency
    select(-condnum) # eliminate variable with conditions as numbers
...
# summarize data
summary.P1.peaks <- summarySEwithin(data.P1.peaks,"peak.amp",withinvars=c("size","cont","emo"),idvar="participant")
kable(summary.P1.peaks,digits=2)

# P1 graph
pirateplot(formula=peak.amp~emo+size+cont, # dependent~independent variables
           data=data.P1.peaks, # data frame
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  ### CONFIRMATORY ANALYSES ###
  ## main effects of size and emotion
  # P1.peakamp.sizeplusemo.BF <- lmBF(peak.amp~size+emo, # formula
  #                                   data.P1.peaks, # data
  #                                   whichRandom=c("participant","word"), # random effects
  #                                   rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                   rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  ### interactive effects of size and emotion
  # P1.peakamp.sizebyemo.BF <- lmBF(peak.amp~size*emo, # formula
  #                                 data.P1.peaks, # data
  #                                 whichRandom=c("participant","word"), # random effects
  #                                 rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                 rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  ### main effects of contrast and emotion
  # P1.peakamp.contplusemo.BF <- lmBF(peak.amp~cont+emo, # formula
  #                                   data.P1.peaks, # data
  #                                   whichRandom=c("participant","word"), # random effects
  #                                   rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                   rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  ### interactive effects of contrast and emotion
  # P1.peakamp.contbyemo.BF <- lmBF(peak.amp~cont*emo, # formula
  #                                 data.P1.peaks, # data
  #                                 whichRandom=c("participant","word"), # random effects
  #                                 rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                 rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  ### main effects of size, contrast, and emotion
  # P1.peakamp.sizepluscontplusemo.BF <- lmBF(peak.amp~size+cont+emo, # formula
  #                                           data.P1.peaks, # data
  #                                           whichRandom=c("participant","word"), # random effects
  #                                           rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                           rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  ### interactive effects of size, contrast, and emotion
  # P1.peakamp.sizebycontbyemo.BF <- lmBF(peak.amp~size*cont*emo, # formula
  #                                       data.P1.peaks, # data
  #                                       whichRandom=c("participant","word"), # random effects
  #                                       rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
  #                                       rscaleRandom="nuisance", # prior scale for standardized random effects
  ...
  # P1.peakamp.anovaBF <- anovaBF(peak.amp~size*cont*emo, # formula
  #                       data.P1.peaks, # data
  #                       whichModels="top", # test all models created by removing a main effect or interaction from the full model
  #                       whichRandom=c("participant","word"), # random effects
  #                       rscaleFixed=scaling.factor[k], # scaling factor of prior on effect size
...
# main effects
# size
P1.peakamp.posthocBF.size <- data.P1.peaks %>% # data frame
  select(participant,component,size,peak.amp) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="peak.amp",withinvars=c("participant","size")) %>% # summary
  split(.$size) # split according to condition
# contrast
P1.peakamp.posthocBF.cont <- data.P1.peaks %>% # data frame
  select(participant,component,cont,peak.amp) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="peak.amp",withinvars=c("participant","cont")) %>% # summary
  split(.$cont) # split according to condition
# emotion
P1.peakamp.posthocBF.emo <- data.P1.peaks %>% # data frame
  select(participant,component,emo,peak.amp) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="peak.amp",withinvars=c("participant","emo")) %>% # summary
  split(.$emo) # split according to condition
# interactions of interest (i.e., always negative vs. neutral)
P1.peakamp.posthocBF <- data.P1.peaks %>% # data frame
  select(participant,condition,peak.amp) %>% # select only variables of interest
  split(.$condition) # split according to condition
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  # large vs. small
  # P1.peakamp.posthocBF.largeVSsmall <- ttestBF(P1.peakamp.posthocBF.size$large$peak.amp, # first condition
  #                                              P1.peakamp.posthocBF.size$small$peak.amp, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  ...
  # dark vs. bright
  # P1.peakamp.posthocBF.darkVSbright <- ttestBF(P1.peakamp.posthocBF.cont$dark$peak.amp, # first condition
  #                                              P1.peakamp.posthocBF.cont$bright$peak.amp, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  ...
  # negative vs. neutral
  # P1.peakamp.posthocBF.negVSneut <- ttestBF(P1.peakamp.posthocBF.emo$neutral$peak.amp, # first condition
  #                                           P1.peakamp.posthocBF.emo$negative$peak.amp, # second condition
  #                                           mu=0, # null hypothesis (mean difference=0)
  #                                           paired=TRUE, # paired sample test
  #                                           iterations=niter, # number of MCMC iterations
  #                                           rscale=scaling.factor[k]) # scaling factor
  ...
  # large, dark, negative vs. neutral
  # P1.peakamp.posthocBF.large.dark.negVSneut <- ttestBF(P1.peakamp.posthocBF$negativeLargeDark$peak.amp, # first condition
  #                                                      P1.peakamp.posthocBF$neutralLargeDark$peak.amp, # second condition
  #                                                      mu=0, # null hypothesis (mean difference=0)
  #                                                      paired=TRUE, # paired sample test
  #                                                      iterations=niter, # number of MCMC iterations
  #                                                      rscale=scaling.factor[k]) # scaling factor
  ...
  # small, dark, negative vs. neutral
  # P1.peakamp.posthocBF.small.dark.negVSneut <- ttestBF(P1.peakamp.posthocBF$negativeSmallDark$peak.amp, # first condition
  #                                                      P1.peakamp.posthocBF$neutralSmallDark$peak.amp, # second condition
  #                                                      mu=0, # null hypothesis (mean difference=0)
  #                                                      paired=TRUE, # paired sample test
  #                                                      iterations=niter, # number of MCMC iterations
  #                                                      rscale=scaling.factor[k]) # scaling factor
  ...
  # large, bright, negative vs. neutral
  # P1.peakamp.posthocBF.large.bright.negVSneut <- ttestBF(P1.peakamp.posthocBF$negativeLargeBright$peak.amp, # first condition
  #                                                P1.peakamp.posthocBF$neutralLargeBright$peak.amp, # second condition
  #                                                mu=0, # null hypothesis (mean difference=0)
  #                                                paired=TRUE, # paired sample test
  #                                                iterations=niter, # number of MCMC iterations
  #                                                rscale=scaling.factor[k]) # scaling factor
  ...
  # small, bright, negative vs. neutral
  # P1.peakamp.posthocBF.small.bright.negVSneut <- ttestBF(P1.peakamp.posthocBF$negativeSmallBright$peak.amp, # first condition
  #                                                P1.peakamp.posthocBF$neutralSmallBright$peak.amp, # second condition
  #                                                mu=0, # null hypothesis (mean difference=0)
  #                                                iterations=niter, # number of MCMC iterations
  #                                                paired=TRUE, # paired sample test
  #                                                rscale=scaling.factor[k]) # scaling factor
