...
# ggplot2 custom color palette for waveform plots
...
grand.avg.topos <- grand.avg.topos %>% # data frame
  ...
  gather(key=component, # name of new column
         ...
         -c(participant,electrode)) # keep participants, electrode cluster, and conditions as columns
```
...
Amplitude extracted from the following electrode cluster: P7 PO7 O1 O2 PO8 P8 P10 P9.   
...
ggplot(filter(P1.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  guides(fill=FALSE,group=FALSE) + # remove unnecessary legends
  ...
  scale_y_reverse(breaks=seq(-3,3,1),limits=c(4,-4)) + # y-axis: tick marks
  geom_vline(xintercept=0,linetype="dashed",color="black",size=1.2,alpha=.8) + # vertical reference line
  geom_hline(yintercept=0,linetype="dashed",color="black",size=1.2,alpha=.8) + # horizontal reference line
  geom_vline(xintercept=seq(from=-200,to=1000,by=50),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  geom_hline(yintercept=seq(from=-3,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=66,xmax=148,ymin=-.5,ymax=3,fill="#e08328",alpha=.2) + # highlight time window used for analysis
  theme_pander(base_size=20,pc="white",lp=c(.15,.8)) # custom theme
```

```{r P1_topo}
P1.topo <- grand.avg.topos %>% # data frame
  filter(component=="P1") %>% # select only component of interest 
...
Amplitude extracted from the following electrode cluster: T7 TP7 P7 PO7 T8 TP8 P8 PO8 P10 P9.
...
ggplot(filter(N1.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  geom_vline(xintercept=seq(from=-200,to=1000,by=50),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  geom_hline(yintercept=seq(from=-3,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=150,xmax=260,ymin=-2,ymax=1,fill="#e08328",alpha=.2) + # highlight time window used for analysis
  theme_pander(base_size=20,pc="white",lp=c(.15,.8)) # custom theme
```

```{r N1_topo}
N1.topo <- grand.avg.topos %>% # data frame
  filter(component=="N1") %>% # select only component of interest 
  summarySEwithin(data=.,measurevar="amplitude",withinvars="electrode",idvar="participant") # summary
...
ggplot(subset(comps.grand.avg.allbins,component=="EPN" & bin!="all"),aes(timepoint,amplitude)) + # basic plot
  ...
  theme_pander(base_size=20,pc="white",lp=c(.2,.8)) # custom theme
```
...
ggplot(EPN.grand.avg.allbins,aes(timepoint,amplitude)) + # basic plot
  ...
  theme_pander(base_size=20,pc="white",lp=c(.2,.3)) # custom theme
```
...
temp <- EPN.N1.grand.avg.allbins %>%
  select(emo,amplitude,timepoint) %>% # select columns of interest
  spread(emo,amplitude) %>% # convert to wide format
  mutate(diff=negative-neutral) # calculate amplitude difference difference
