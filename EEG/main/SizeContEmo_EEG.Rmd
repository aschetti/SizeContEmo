...
grand.avg.localizer <- grand.avg.localizer %>% # data frame
  select(-bin) %>% # drop bin column
  # convert to long format
  gather(key=timepoint, # name of new column
         value=amplitude, # name of new column
...
# plot
ggplot(grand.avg.localizer.pointsummary,aes(x=timepoint,y=amplitude,group=electrode,color=electrode)) + # basic plot
  geom_line(aes(color=electrode),size=1.2) + # one line per electrode
  scale_color_viridis(discrete=TRUE,option="magma",alpha=.7) + # color scale
