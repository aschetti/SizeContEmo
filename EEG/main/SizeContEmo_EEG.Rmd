...
ggplot(
  ...
  labs(
    title = "grand average collapsed localizer (all electrodes)", # title & axes labels
    x = "time (ms)",
...
P1.grand.avg.comps.pointsummary <- grand.avg.comps %>% # data frame
  ...
  unite(visualfeats, c("size", "cont")) %>% # for graph
  mutate(visualfeats = as.factor(ifelse(visualfeats == "NA_NA", "localizer", visualfeats)),
...
ggplot(
  ...
  scale_color_manual(
    breaks = c(
      "localizer", "large_bright", # re-order conditions
      "large_dark", "small_bright", "small_dark"
    ),
...
#######################################################################
# - DO ALL WAVEFORMS (NO LEGENDS)
# - DO ALL PIRATEPLOTS
# - MERGE WITH COWPLOT OR PATCHWORK
# - LOAD AND OVERLAY TOPOGRAPHY ON WAVEFORMS
#######################################################################
...
data.EEG.trial <- data.EEG.trial %>% # data frame
  mutate(
    condition = factor(condition), # convert condition numbers as factors
    # variable with main effect of size
    size = revalue(condition, c(
      "1" = "large",
      "2" = "small",
      "3" = "large",
      "4" = "small",
    ...
    word = data.EEG.trialWords[, "word"]
  ) %>% # include words in EEG data frame
...
pirateplot(
  formula = P1 ~ emo + size + cont, # dependent~independent variables
...
pirateplot(
  formula = N1 ~ emo + size + cont, # dependent~independent variables
...
pirateplot(
  formula = EPN ~ emo + size + cont, # dependent~independent variables
...
pirateplot(
  formula = LPP ~ emo + size + cont, # dependent~independent variables
