...
ggplot(filter(P1.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  geom_hline(yintercept=seq(from=-3,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=66,xmax=148,ymin=-.5,ymax=3,linetype=2,size=2,color="#de1d1d",alpha=0) + # highlight time window used for analysis
...
ggplot(filter(N1.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  geom_vline(xintercept=seq(from=-200,to=1000,by=50),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  geom_hline(yintercept=seq(from=-3,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=150,xmax=260,ymin=-2.5,ymax=1,linetype=2,size=2,color="#de1d1d",alpha=0) + # highlight time window used for analysis
  theme_pander(base_size=20,pc="white",lp=c(.25,.8)) # custom theme
```
...
ggplot(EPN.grand.avg.comps.pointsummary,aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  geom_line(aes(color=condition),size=2.5,alpha=.6) + # one line per condition
  ...
  scale_y_reverse(breaks=seq(-2,3,1),limits=c(3,-2)) + # y-axis: tick marks
  geom_vline(xintercept=0,linetype="dashed",color="black",size=1.2,alpha=.8) + # vertical reference line
  geom_hline(yintercept=0,linetype="dashed",color="black",size=1.2,alpha=.8) + # horizontal reference line
  geom_vline(xintercept=seq(from=-200,to=1000,by=50),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  geom_hline(yintercept=seq(from=-2,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=300,xmax=500,ymin=-1,ymax=2,linetype=2,size=2,color="#de1d1d",alpha=0) + # highlight time window used for analysis
  theme_pander(base_size=20,pc="white",lp=c(.25,.8)) # custom theme
```
...
ggplot(filter(LPP.grand.avg.comps.pointsummary,condition!="localizer"),aes(x=timepoint,y=amplitude,group=condition,color=condition)) + # basic plot
  ...
  geom_hline(yintercept=seq(from=-3,to=3,by=1),linetype="dotted",color="#999999",size=.8,alpha=.5) + # reference lines
  annotate("rect",xmin=402,xmax=684,ymin=-.5,ymax=2.5,linetype=2,size=2,color="#de1d1d",alpha=0) + # highlight time window used for analysis
  theme_pander(base_size=20,pc="white",lp=c(.25,.8)) # custom theme
```
...
# pirateplot
pirateplot(formula=value~emo+size+cont, # dependent~independent variables
...
P1.peakamp.posthocBF.size <- subset(data.peaks,component=="P1" & var=="peak.amp") %>% # data frame
  select(participant,component,size,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","size")) %>% # summary
  split(.$size) # split according to condition
# contrast
P1.peakamp.posthocBF.cont <- subset(data.peaks,component=="P1" & var=="peak.amp") %>% # data frame
  select(participant,component,cont,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","cont")) %>% # summary
  split(.$cont) # split according to condition
# emotion
P1.peakamp.posthocBF.emo <- subset(data.peaks,component=="P1" & var=="peak.amp") %>% # data frame
  select(participant,component,emo,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","emo")) %>% # summary
  split(.$emo) # split according to condition
# interactions of interest (i.e., always negative vs. neutral)
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  # large vs. small
  # P1.peakamp.posthocBF.largeVSsmall <- ttestBF(P1.peakamp.posthocBF.size$large$value, # first condition
  #                                              P1.peakamp.posthocBF.size$small$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  ...
  # dark vs. bright
  # P1.peakamp.posthocBF.darkVSbright <- ttestBF(P1.peakamp.posthocBF.cont$dark$value, # first condition
  #                                              P1.peakamp.posthocBF.cont$bright$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  ...
  # negative vs. neutral
  # P1.peakamp.posthocBF.negVSneut <- ttestBF(P1.peakamp.posthocBF.emo$neutral$value, # first condition
  #                                           P1.peakamp.posthocBF.emo$negative$value, # second condition
  #                                           mu=0, # null hypothesis (mean difference=0)
  #                                           paired=TRUE, # paired sample test
  #                                           iterations=niter, # number of MCMC iterations
  #                                           rscale=scaling.factor[k]) # scaling factor
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  ### model comparison across scaling factors
  # BFs
  compare.P1.peaklat.BF[,k] <- c(exp(P1.peaklat.sizeplusemo.BF@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.P1.peaklat.perc.err[,k] <- c(P1.peaklat.sizeplusemo.BF@bayesFactor$error[1]*100,
...
# summary confirmatory analysis
compare.P1.peaklat <- data.frame("model"=c("size + emo","size x emo","contr + emo","cont x emo","size + cont + emo","size x cont x emo"),
...
# summary exploratory analysis
compare.P1.peaklat.anovaBF <- data.frame("omit from full model"=c("cont:emo:size","cont:emo","emo:size","cont:size","emo","cont","size"),
...
compare.P1.peaklat.anovaBF <- compare.P1.peaklat.anovaBF[order(compare.P1.peaklat.anovaBF$nar,decreasing=TRUE),] # sort according to medium scaling factor (in descending order)
```
...
# main effects
# size
P1.peaklat.posthocBF.size <- subset(data.peaks,component=="P1" & var=="peak.lat") %>% # data frame
  select(participant,component,size,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","size")) %>% # summary
  split(.$size) # split according to condition
# contrast
P1.peaklat.posthocBF.cont <- subset(data.peaks,component=="P1" & var=="peak.lat") %>% # data frame
  select(participant,component,cont,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","cont")) %>% # summary
  split(.$cont) # split according to condition
# emotion
P1.peaklat.posthocBF.emo <- subset(data.peaks,component=="P1" & var=="peak.lat") %>% # data frame
  select(participant,component,emo,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","emo")) %>% # summary
  split(.$emo) # split according to condition
# interactions of interest (i.e., always negative vs. neutral)
...
compare.P1.peaklat.posthocBF <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all BF10
compare.P1.peaklat.posthocBF.perc.err <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all % errors

for(k in 1:length(scaling.factor)) { # loop through scaling factors

  # large vs. small
  # P1.peaklat.posthocBF.largeVSsmall <- ttestBF(P1.peaklat.posthocBF.size$large$value, # first condition
  #                                              P1.peaklat.posthocBF.size$small$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(P1.peaklat.posthocBF.largeVSsmall,file=paste0(wd,"/models/P1_peaklat_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # save model
  P1.peaklat.posthocBF.largeVSsmall <- readRDS(paste0(wd,"/models/P1_peaklat_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # load model

  # dark vs. bright
  # P1.peaklat.posthocBF.darkVSbright <- ttestBF(P1.peaklat.posthocBF.cont$dark$value, # first condition
  #                                              P1.peaklat.posthocBF.cont$bright$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(P1.peaklat.posthocBF.darkVSbright,file=paste0(wd,"/models/P1_peaklat_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # save model
  P1.peaklat.posthocBF.darkVSbright <- readRDS(paste0(wd,"/models/P1_peaklat_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # load model

  # negative vs. neutral
  # P1.peaklat.posthocBF.negVSneut <- ttestBF(P1.peaklat.posthocBF.emo$neutral$value, # first condition
  #                                           P1.peaklat.posthocBF.emo$negative$value, # second condition
  #                                           mu=0, # null hypothesis (mean difference=0)
  #                                           paired=TRUE, # paired sample test
  #                                           iterations=niter, # number of MCMC iterations
  #                                           rscale=scaling.factor[k]) # scaling factor
  # saveRDS(P1.peaklat.posthocBF.negVSneut,file=paste0(wd,"/models/P1_peaklat_posthocBF_negVSneut_",scaling.factor[k],".rds")) # save model
  P1.peaklat.posthocBF.negVSneut <- readRDS(paste0(wd,"/models/P1_peaklat_posthocBF_negVSneut_",scaling.factor[k],".rds")) # load model
  ...
  ### model comparison
  compare.P1.peaklat.posthocBF[,k] <- c(exp(P1.peaklat.posthocBF.largeVSsmall@bayesFactor$bf[1]),
                                exp(P1.peaklat.posthocBF.darkVSbright@bayesFactor$bf[1]),
                                exp(P1.peaklat.posthocBF.negVSneut@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.P1.peaklat.posthocBF.perc.err[,k] <- c(P1.peaklat.posthocBF.largeVSsmall@bayesFactor$error[1]*100,
                                         P1.peaklat.posthocBF.darkVSbright@bayesFactor$error[1]*100,
                                         P1.peaklat.posthocBF.negVSneut@bayesFactor$error[1]*100,
...
# summary
compare.P1.peaklat.posthocBF <- data.frame("comparison"=c("largeVSsmall","darkVSbright","negVSneut","large.dark.negVSneut","small.dark.negVSneut","large.bright.negVSneut","small.bright.negVSneut"),
...
Follow-up contrasts reveal that the main effect of *contrast* best explains changes in P1 peak latency. Exploratory analyses further suggest that omitting the factors *size* or *emotion* from the full model improves fitting by at least `r compare.P1.peaklat.anovaBF$nar[1]` and `r compare.P1.peaklat.anovaBF$nar[2]` times. Removing the interactions *emotion x contrast*, *emotion x size*, and *size x  contrast x emotion* also improves fitting by at least `r compare.P1.peaklat.anovaBF$nar[3]`, `r compare.P1.peaklat.anovaBF$nar[4]`, and `r compare.P1.peaklat.anovaBF$nar[5]` times, respectively.   
Conversely, omitting the *contrast x size* interaction lowers the explanatory value of the resulting model by at least `r 1/compare.P1.peaklat.anovaBF$nar[6]` times. Finally, omitting the factor *contrast* is maximally detrimental, as it would lower the explanatory value of the resulting model by at least `r 1/compare.P1.peaklat.anovaBF$nar[7]` times.
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  ### model comparison across scaling factors
  # BFs
  compare.N1.peakamp.BF[,k] <- c(exp(N1.peakamp.sizeplusemo.BF@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.N1.peakamp.perc.err[,k] <- c(N1.peakamp.sizeplusemo.BF@bayesFactor$error[1]*100,
...
# summary confirmatory analysis
compare.N1.peakamp <- data.frame("model"=c("size + emo","size x emo","contr + emo","cont x emo","size + cont + emo","size x cont x emo"),
...
# summary exploratory analysis
compare.N1.peakamp.anovaBF <- data.frame("omit from full model"=c("cont:emo:size","cont:emo","emo:size","cont:size","emo","cont","size"),
...
compare.N1.peakamp.anovaBF <- compare.N1.peakamp.anovaBF[order(compare.N1.peakamp.anovaBF$nar,decreasing=TRUE),] # sort according to medium scaling factor (in descending order)
```
...
# main effects
# size
N1.peakamp.posthocBF.size <- subset(data.peaks,component=="N1" & var=="peak.amp") %>% # data frame
  select(participant,component,size,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","size")) %>% # summary
  split(.$size) # split according to condition
# contrast
N1.peakamp.posthocBF.cont <- subset(data.peaks,component=="N1" & var=="peak.amp") %>% # data frame
  select(participant,component,cont,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","cont")) %>% # summary
  split(.$cont) # split according to condition
# emotion
N1.peakamp.posthocBF.emo <- subset(data.peaks,component=="N1" & var=="peak.amp") %>% # data frame
  select(participant,component,emo,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","emo")) %>% # summary
  split(.$emo) # split according to condition
# interactions of interest (i.e., always negative vs. neutral)
...
compare.N1.peakamp.posthocBF <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all BF10
compare.N1.peakamp.posthocBF.perc.err <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all % errors

for(k in 1:length(scaling.factor)) { # loop through scaling factors

  # large vs. small
  # N1.peakamp.posthocBF.largeVSsmall <- ttestBF(N1.peakamp.posthocBF.size$large$value, # first condition
  #                                              N1.peakamp.posthocBF.size$small$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peakamp.posthocBF.largeVSsmall,file=paste0(wd,"/models/N1_peakamp_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # save model
  N1.peakamp.posthocBF.largeVSsmall <- readRDS(paste0(wd,"/models/N1_peakamp_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # load model

  # dark vs. bright
  # N1.peakamp.posthocBF.darkVSbright <- ttestBF(N1.peakamp.posthocBF.cont$dark$value, # first condition
  #                                              N1.peakamp.posthocBF.cont$bright$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peakamp.posthocBF.darkVSbright,file=paste0(wd,"/models/N1_peakamp_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # save model
  N1.peakamp.posthocBF.darkVSbright <- readRDS(paste0(wd,"/models/N1_peakamp_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # load model

  # negative vs. neutral
  # N1.peakamp.posthocBF.negVSneut <- ttestBF(N1.peakamp.posthocBF.emo$neutral$value, # first condition
  #                                           N1.peakamp.posthocBF.emo$negative$value, # second condition
  #                                           mu=0, # null hypothesis (mean difference=0)
  #                                           paired=TRUE, # paired sample test
  #                                           iterations=niter, # number of MCMC iterations
  #                                           rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peakamp.posthocBF.negVSneut,file=paste0(wd,"/models/N1_peakamp_posthocBF_negVSneut_",scaling.factor[k],".rds")) # save model
  N1.peakamp.posthocBF.negVSneut <- readRDS(paste0(wd,"/models/N1_peakamp_posthocBF_negVSneut_",scaling.factor[k],".rds")) # load model
  ...
  ### model comparison
  compare.N1.peakamp.posthocBF[,k] <- c(exp(N1.peakamp.posthocBF.largeVSsmall@bayesFactor$bf[1]),
                                exp(N1.peakamp.posthocBF.darkVSbright@bayesFactor$bf[1]),
                                exp(N1.peakamp.posthocBF.negVSneut@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.N1.peakamp.posthocBF.perc.err[,k] <- c(N1.peakamp.posthocBF.largeVSsmall@bayesFactor$error[1]*100,
                                         N1.peakamp.posthocBF.darkVSbright@bayesFactor$error[1]*100,
                                         N1.peakamp.posthocBF.negVSneut@bayesFactor$error[1]*100,
...
# summary
compare.N1.peakamp.posthocBF <- data.frame("comparison"=c("largeVSsmall","darkVSbright","negVSneut","large.dark.negVSneut","small.dark.negVSneut","large.bright.negVSneut","small.bright.negVSneut"),
...
Follow-up contrasts reveal that the main effect of *contrast* best explains changes in N1 peak amplitude, closely followed by the main effect of *size*. Exploratory analyses further suggest that omitting the factor *emotion* from the full model improves fitting by at least `r compare.N1.peakamp.anovaBF$nar[1]` times. Removing the interactions *emotion x contrast*, *emotion x size*, and *size x  contrast x emotion* also improves fitting by at least `r compare.N1.peakamp.anovaBF$nar[2]`, `r compare.N1.peakamp.anovaBF$nar[3]`, and `r compare.N1.peakamp.anovaBF$nar[4]` times, respectively.   
Conversely, omitting the *contrast x size* interaction or the factor *size* lowers the explanatory value of the resulting model by at least `r 1/compare.N1.peakamp.anovaBF$nar[5]` and `r 1/compare.N1.peakamp.anovaBF$nar[6]` times. Finally, omitting the factor *contrast* is maximally detrimental, as it would lower the explanatory value of the resulting model by at least `r 1/compare.N1.peakamp.anovaBF$nar[7]` times.
...
for(k in 1:length(scaling.factor)) { # loop through scaling factors
  ...
  ### model comparison across scaling factors
  # BFs
  compare.N1.peaklat.BF[,k] <- c(exp(N1.peaklat.sizeplusemo.BF@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.N1.peaklat.perc.err[,k] <- c(N1.peaklat.sizeplusemo.BF@bayesFactor$error[1]*100,
...
# summary confirmatory analysis
compare.N1.peaklat <- data.frame("model"=c("size + emo","size x emo","contr + emo","cont x emo","size + cont + emo","size x cont x emo"),
...
# summary exploratory analysis
compare.N1.peaklat.anovaBF <- data.frame("omit from full model"=c("cont:emo:size","cont:emo","emo:size","cont:size","emo","cont","size"),
...
compare.N1.peaklat.anovaBF <- compare.N1.peaklat.anovaBF[order(compare.N1.peaklat.anovaBF$nar,decreasing=TRUE),] # sort according to medium scaling factor (in descending order)
```
...
# main effects
# size
N1.peaklat.posthocBF.size <- subset(data.peaks,component=="N1" & var=="peak.lat") %>% # data frame
  select(participant,component,size,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","size")) %>% # summary
  split(.$size) # split according to condition
# contrast
N1.peaklat.posthocBF.cont <- subset(data.peaks,component=="N1" & var=="peak.lat") %>% # data frame
  select(participant,component,cont,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","cont")) %>% # summary
  split(.$cont) # split according to condition
# emotion
N1.peaklat.posthocBF.emo <- subset(data.peaks,component=="N1" & var=="peak.lat") %>% # data frame
  select(participant,component,emo,value) %>% # select only variables of interest
  summarySEwithin(data=.,measurevar="value",withinvars=c("participant","emo")) %>% # summary
  split(.$emo) # split according to condition
# interactions of interest (i.e., always negative vs. neutral)
...
compare.N1.peaklat.posthocBF <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all BF10
compare.N1.peaklat.posthocBF.perc.err <- matrix(NA,7,length(scaling.factor)) # preallocate matrix with all % errors

for(k in 1:length(scaling.factor)) { # loop through scaling factors

  # large vs. small
  # N1.peaklat.posthocBF.largeVSsmall <- ttestBF(N1.peaklat.posthocBF.size$large$value, # first condition
  #                                              N1.peaklat.posthocBF.size$small$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peaklat.posthocBF.largeVSsmall,file=paste0(wd,"/models/N1_peaklat_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # save model
  N1.peaklat.posthocBF.largeVSsmall <- readRDS(paste0(wd,"/models/N1_peaklat_posthocBF_largeVSsmall_",scaling.factor[k],".rds")) # load model

  # dark vs. bright
  # N1.peaklat.posthocBF.darkVSbright <- ttestBF(N1.peaklat.posthocBF.cont$dark$value, # first condition
  #                                              N1.peaklat.posthocBF.cont$bright$value, # second condition
  #                                              mu=0, # null hypothesis (mean difference=0)
  #                                              paired=TRUE, # paired sample test
  #                                              iterations=niter, # number of MCMC iterations
  #                                              rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peaklat.posthocBF.darkVSbright,file=paste0(wd,"/models/N1_peaklat_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # save model
  N1.peaklat.posthocBF.darkVSbright <- readRDS(paste0(wd,"/models/N1_peaklat_posthocBF_darkVSbright_",scaling.factor[k],".rds")) # load model

  # negative vs. neutral
  # N1.peaklat.posthocBF.negVSneut <- ttestBF(N1.peaklat.posthocBF.emo$neutral$value, # first condition
  #                                           N1.peaklat.posthocBF.emo$negative$value, # second condition
  #                                           mu=0, # null hypothesis (mean difference=0)
  #                                           paired=TRUE, # paired sample test
  #                                           iterations=niter, # number of MCMC iterations
  #                                           rscale=scaling.factor[k]) # scaling factor
  # saveRDS(N1.peaklat.posthocBF.negVSneut,file=paste0(wd,"/models/N1_peaklat_posthocBF_negVSneut_",scaling.factor[k],".rds")) # save model
  N1.peaklat.posthocBF.negVSneut <- readRDS(paste0(wd,"/models/N1_peaklat_posthocBF_negVSneut_",scaling.factor[k],".rds")) # load model
  ...
  ### model comparison
  compare.N1.peaklat.posthocBF[,k] <- c(exp(N1.peaklat.posthocBF.largeVSsmall@bayesFactor$bf[1]),
                                exp(N1.peaklat.posthocBF.darkVSbright@bayesFactor$bf[1]),
                                exp(N1.peaklat.posthocBF.negVSneut@bayesFactor$bf[1]),
  ...
  # percentage of error
  compare.N1.peaklat.posthocBF.perc.err[,k] <- c(N1.peaklat.posthocBF.largeVSsmall@bayesFactor$error[1]*100,
                                         N1.peaklat.posthocBF.darkVSbright@bayesFactor$error[1]*100,
                                         N1.peaklat.posthocBF.negVSneut@bayesFactor$error[1]*100,
...
# summary
compare.N1.peaklat.posthocBF <- data.frame("comparison"=c("largeVSsmall","darkVSbright","negVSneut","large.dark.negVSneut","small.dark.negVSneut","large.bright.negVSneut","small.bright.negVSneut"),
...
Follow-up contrasts reveal that the main effect of *contrast* best explains changes in N1 peak latency. Exploratory analyses further suggest that omitting the factor *emotion* from the full model improves fitting by at least `r compare.N1.peaklat.anovaBF$nar[1]` times. Removing the interactions *contrast x size*, *contrast x emotion*, *size x  contrast x emotion*, and *emotion x size* also improves fitting by at least `r compare.N1.peaklat.anovaBF$nar[2]`, `r compare.N1.peaklat.anovaBF$nar[3]`,`r compare.N1.peaklat.anovaBF$nar[4]`, and `r compare.N1.peaklat.anovaBF$nar[5]` times, respectively.   
Conversely, omitting the factor *size* lowers the explanatory value of the resulting model by at least `r 1/compare.N1.peaklat.anovaBF$nar[6]` times. Finally, omitting the factor *contrast* is maximally detrimental, as it would lower the explanatory value of the resulting model by at least `r 1/compare.N1.peaklat.anovaBF$nar[7]` times.
